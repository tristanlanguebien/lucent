...
class MyConventions(Conventions):
    # REMOVE THESE BEFORE OFFICIAL LUCENT RELEASE
    # These are conventions only designed to be used as configuration for entities fields
    asset_identifier = Convention('{asset}')
    asset_fields = Convention('{asset}_{type}_{group}')
    shot_identifier = Convention('{season}_{episode}_{shot}')
    shot_fields = Convention('{season}_{episode}_{shot}_{sequence}')
    ...
    asset_publish_file = Convention('{@asset_dir}/publish/v{version}/someSubdir/{asset}_v{version}.{extension}', fixed_fields={'extension': 'ma'})
